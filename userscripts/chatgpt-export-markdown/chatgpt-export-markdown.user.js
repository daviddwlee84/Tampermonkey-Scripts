...
// @version      1.4.0
// @description  把 ChatGPT 對話匯成 Markdown，另提供樹狀選取、Agent Handoff 與可攜 JSON 存檔
// @author       Da-Wei Lee
// @license      MIT
...
// @require      https://raw.githubusercontent.com/daviddwlee84/Tampermonkey-Scripts/main/shared/chatgpt-adapter.js
// @require      https://raw.githubusercontent.com/daviddwlee84/Tampermonkey-Scripts/main/shared/conversation-archive.js
// @require      https://raw.githubusercontent.com/daviddwlee84/Tampermonkey-Scripts/main/shared/vendor/marked-18.0.13.umd.js
// @require      https://raw.githubusercontent.com/daviddwlee84/Tampermonkey-Scripts/main/shared/vendor/dompurify-3.4.15.min.js
// @require      https://raw.githubusercontent.com/daviddwlee84/Tampermonkey-Scripts/main/shared/conversation-explorer.js
...
/* global createExportPanel, downloadText, filenameFor, renderTranscript, ChatGPTAdapter, ConversationArchive, createConversationExplorer */
...
(function () {
  ...
  async function fromBackendApi(convId, shareId, { signal } = {}) {
    let authorization = null;
    try {
      const session = await fetch('/api/auth/session', { credentials: 'include', signal });
      if (session.ok) {
    ...
    for (const path of paths) {
      try {
        ...
        const res = await fetch(path, { credentials: 'include', headers, signal });
        if (!res.ok) continue;
    ...
    try {
      ...
      const doc =
        mode === 'json'
          ...
          : ChatGPTAdapter.normalize(data, { url }, settings);
      const text =
        mode === 'json'
  ...
  // 瀏覽模式的快照、設定與勾選不回寫 captured / settings，也不改動網站目前的 branch。
  let explorer = null;
  let explorerRoute = null;
  let explorerWatch = null;
  ...
  function watchExplorerRoute() {
    clearInterval(explorerWatch);
    explorerWatch = setInterval(() => {
      if (explorerRoute !== location.href) disposeExplorer();
    }, 250);
  }

  async function openExplorer() {
    if (explorer && explorerRoute === location.href) {
      explorer.open();
      watchExplorerRoute();
      return;
    }
    ...
    explorerRoute = route;
    const current = createConversationExplorer({
      copyText: (text) => GM_setClipboard(text, 'text'),
      onClose: () => {
        clearInterval(explorerWatch);
        explorerAbort?.abort();
      },
      onRefresh: async () => {
        ...
        const stillCurrent = () => explorer === current && location.href === route;
        let resolved;
        try {
          // 僅新模式優先更新登入中的對話；一般一鍵匯出的解析順序維持原樣。
          const fresh = target.convId
            ? await fromBackendApi(target.convId, null, { signal: controller.signal })
            : null;
          if (!stillCurrent() || (controller.signal.aborted && !timedOut))
            throw new Error('已取消讀取舊頁面的對話。');
          resolved = fresh
            ? { data: fresh, source: 'backend-api' }
            : await resolveConversation(target);
        } finally {
          clearTimeout(timer);
        }
        if (!stillCurrent() || (controller.signal.aborted && !timedOut))
          throw new Error('已取消讀取舊頁面的對話。');
        return ConversationArchive.fromChatGPT(resolved.data, {
          url: route,
          type: resolved.source,
          options: {
            showTools: settings.includeTools,
            showThinking: settings.includeThinking,
            includeResearchDetails: settings.includeResearchDetails,
          },
        });
      },
    });
